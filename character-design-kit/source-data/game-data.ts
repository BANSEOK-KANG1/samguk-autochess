...
export const BATTLEFIELD_THEMES: BattlefieldRule[] = [
  ...
  },
  {
    ...
    accent: "#87949a",
    ruleText: ["궁수·수호 전투력 +16%", "기병 전투력 -10%"],
  },
  {
    id: "바다",
    slug: "sea",
...
export const ROLE_EFFECTS: Record<Role, { tiers: number[]; effect: string }> = {
  군주: { tiers: [2, 4], effect: "아군 전체 능력치 상승" },
  용장: { tiers: [2, 4, 6], effect: "공격력과 흡혈 상승" },
  수호: { tiers: [2, 4, 6], effect: "방어력과 피해 감소 상승" },
  책사: { tiers: [2, 4, 6], effect: "책략 피해와 기력 회복 상승" },
  궁수: { tiers: [2, 4], effect: "사거리와 치명타 확률 상승" },
  기병: { tiers: [2, 4], effect: "전투 시작 돌진 및 공격 속도 상승" },
  암살: { tiers: [2, 4], effect: "후열 침투와 치명타 피해 상승" },
  지원: { tiers: [2, 4], effect: "회복·보호 효과 상승" },
};

export const BOND_RULES: Record<string, { tiers: number[]; effect: string }> = {
  도원결의: { tiers: [3], effect: "한 명이 쓰러지면 형제의 공격력·기력 상승" },
  오호대장군: { tiers: [2, 5], effect: "오호 장수의 공격력과 피해 감소 상승" },
  와룡봉추: { tiers: [2], effect: "첫 책략이 두 번 발동" },
  손씨일가: { tiers: [2, 4], effect: "상점 새로고침 시 오 장수 출현 확률 상승" },
  강동쌍벽: { tiers: [2], effect: "손책이 돌진하면 주유가 화공 지원" },
  이교: { tiers: [2], effect: "매 전투 처음 받는 치명 피해를 무효화" },
  강동사걸: { tiers: [2, 4], effect: "책략 사용 시 아군 오 장수의 기력 회복" },
  오자양장: { tiers: [2, 5], effect: "위 장수 처치 관여 시 공격 속도 중첩" },
  하후일문: { tiers: [2], effect: "전열과 후열이 서로 피해를 분담" },
  호위쌍벽: { tiers: [2], effect: "가장 강한 군주를 대신해 피해를 받음" },
  왕좌지재: { tiers: [2, 4], effect: "전투 시작 시 무작위 적의 능력치 약화" },
  비장연가: { tiers: [2], effect: "초선이 생존한 동안 여포가 광폭화" },
  황건형제: { tiers: [3], effect: "쓰러진 황건 병사가 번개를 남김" },
  남만왕가: { tiers: [2], effect: "제어 면역과 지속 회복 획득" },
  하북쌍웅: { tiers: [2], effect: "첫 공격이 반드시 치명타" },
  서량기병: { tiers: [2, 4], effect: "돌진 거리에 비례해 추가 피해" },
  관씨일문: { tiers: [2, 4], effect: "관우의 공격을 관씨 장수가 따라 공격" },
  연환계: { tiers: [2], effect: "적이 받는 제어 효과가 인접 적에게 전이" },
  책략가: { tiers: [2, 4], effect: "전투 시작 시 무작위 계략 획득" },
};
